...
Pawn::getLegalMoves(Board &board) {
    ...
    if (color == WHITE) {
        ...
        if (row == 4 && board.board[row][column]->type == EMPTY &&
            board.board[row + 1][column]->type == EMPTY) {
            Position to(row, column);
            moves.insert({position, to});
        }
        ...
        row = position.row + 2;
        column = position.column;
        if (row == 3 && board.board[row][column]->type == EMPTY &&
            board.board[row - 1][column]->type == EMPTY) {
            Position to(row, column);
            moves.insert({position, to});
        }
    ...
    if (position.row == board.enPassant.row &&
        std::abs(position.column - board.enPassant.column) == 1) {
        Position to(position.row - (color == WHITE ? 1 : -1),
...
void
Pawn::render(SDL_Rect *src, SDL_Rect *dest) {
