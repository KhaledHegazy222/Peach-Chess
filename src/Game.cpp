...
// const char* fen = "r6k/pn2bprp/4pNp1/2p1PbQ1/3p1P2/5NR1/PPP3PP/2B2RK1 w - - 0
// 1";
...
void
Game::init(const char *title, int x, int y, int w, int h) {
...
Game::handleEvents() {
    ...
    switch (event.type) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            board->hold((mouseY - PaddingTop) / TileDimension,
                        (mouseX - PaddingLeft) / TileDimension);
            break;
        case SDL_MOUSEBUTTONUP:
            SDL_GetMouseState(&mouseX, &mouseY);
            board->release((mouseY - PaddingTop) / TileDimension,
                           (mouseX - PaddingLeft) / TileDimension);
            break;
        default:
            break;
