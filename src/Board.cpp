    ...
    for (int row = 0; row < 8; row++) {
        for (int column = 0; column < 8; column++) {
            switch (other.board[row][column]->type) {
                ...
                case KING:
                    board[row][column] =
                        new King(row, column, other.board[row][column]->color);
                    break;
                case QUEEN:
                    board[row][column] =
                        new Queen(row, column, other.board[row][column]->color);
                    break;
                case KNIGHT:
                    board[row][column] = new Knight(
                        row, column, other.board[row][column]->color);
                    break;
                case BISHOP:
                    board[row][column] = new Bishop(
                        row, column, other.board[row][column]->color);
                    break;
                case ROOK:
                    board[row][column] =
                        new Rook(row, column, other.board[row][column]->color);
                    break;
                case PAWN:
                    board[row][column] =
                        new Pawn(row, column, other.board[row][column]->color);
                    break;
                default:
                    break;
            }
...
Board::render() {
    ...
    for (int row = 0; row < 8; row++) {
        for (int column = 0; column < 8; column++) {
            ...
            TextureManager::drawTexture((row + column) % 2 == 0 ? WTile : BTile,
                                        NULL, &dest);
        }
    }
    ...
    if (movesList.size() &&
        validPosition(movesList.top().first->position.row,
                      movesList.top().first->position.column)) {
        SDL_Rect dest;
        dest.x = Game::PaddingLeft +
                 movesList.top().first->position.column * Game::TileDimension;
        dest.y = Game::PaddingTop +
                 movesList.top().first->position.row * Game::TileDimension;
        dest.h = dest.w = Game::TileDimension;
        TextureManager::drawTexture((movesList.top().first->position.row +
                                     movesList.top().first->position.column) %
                                                2 ==
                                            0
                                        ? WLastMove
    ...
    if (movesList.size() &&
        validPosition(movesList.top().second->position.row,
                      movesList.top().second->position.column)) {
        SDL_Rect dest;
        dest.x = Game::PaddingLeft +
                 movesList.top().second->position.column * Game::TileDimension;
        dest.y = Game::PaddingTop +
                 movesList.top().second->position.row * Game::TileDimension;
        dest.h = dest.w = Game::TileDimension;
        TextureManager::drawTexture((movesList.top().second->position.row +
                                     movesList.top().second->position.column) %
                                                2 ==
                                            0
                                        ? WLastMove
    ...
    for (auto move : availableMoves) {
        SDL_Rect dest;
        if (board[move.second.row][move.second.column]->type != EMPTY &&
            board[move.second.row][move.second.column]->color != turn) {
            dest.x =
                Game::PaddingLeft + move.second.column * Game::TileDimension;
            dest.y = Game::PaddingTop + move.second.row * Game::TileDimension;
            dest.h = dest.w = Game::TileDimension;
            TextureManager::drawTexture(TakeTile, NULL, &dest);
        } else {
            dest.x = Game::PaddingLeft +
                     move.second.column * Game::TileDimension +
                     Game::TileDimension / 3;
            dest.y = Game::PaddingTop + move.second.row * Game::TileDimension +
                     Game::TileDimension / 3;
...
void
Board::hold(int row, int column) {
    MovingPiece = NULL;
    if (!validPosition(row, column) || board[row][column]->type == EMPTY ||
        board[row][column]->color != turn) {
        return;
    }
...
bool
Board::validPosition(int row, int column) {
...
bool
Board::isAttacked(Position cell) {
    for (int dx : {-2, -1, 1, 2}) {
        for (int dy : {-2, -1, 1, 2}) {
                ...
                if (board[row][column]->type != EMPTY) {
                    if (board[row][column]->color == turn)
                        break;
                             ...
                             (board[row][column]->type == QUEEN ||
                              board[row][column]->type == BISHOP))
                        return true;
                    else if ((!i || !j) && (board[row][column]->type == QUEEN ||
                                            board[row][column]->type == ROOK))
                        return true;
                    else
                        break;
                }
    ...
    for (int i = -1; i <= 1; i += 2) {
        ...
        if (validPosition(row, column) && board[row][column]->type == PAWN &&
            board[row][column]->color != turn)
            return true;
    }
    return false;
...
void
Board::performMove(Position from, Position to) {
    ...
    BoardInfo currentInfo(CastleWhiteKingSide, CastleWhiteQueenSide,
                          CastleBlackKingSide, CastleBlackQueenSide,
                          WhiteInCheck, BlackInCheck, enPassant);
    info.push(currentInfo);

    // Performing En Passant Move
    if (board[from.row][from.column]->type == PAWN &&
        enPassant.row == from.row && to.column == enPassant.column) {
        delete board[enPassant.row][enPassant.column];
        board[enPassant.row][enPassant.column] =
            new Piece(enPassant.row, enPassant.column);
    }
    ...
    if ((to.row == 0 || to.row == 7) &&
        board[to.row][to.column]->type == PAWN) {
        delete board[to.row][to.column];
    ...
    if (board[to.row][to.column]->type == KING &&
        std::abs(from.column - to.column) > 1) {
        if (to.column == 6) {
...
Board::reverseMove() {
    ...
    if (from->type == PAWN && to->position.column != from->position.column &&
        to->type == EMPTY) {
        delete board[to->position.row - (from->color == WHITE ? -1 : 1)]
                    [to->position.column];
        board[to->position.row - (from->color == WHITE ? -1 : 1)]
             [to->position.column] = new Pawn(
                 to->position.row - (from->color == WHITE ? -1 : 1),
                 to->position.column, (from->color == WHITE ? BLACK : WHITE));
    }
    ...
    if (board[from->position.row][from->position.column]->type == KING &&
        std::abs(from->position.column - to->position.column) > 1) {
...
clone(Piece *cell) {
    switch (cell->type) {
        case EMPTY:
            ...
            break;
        case KING:
            return new King(cell->position.row, cell->position.column,
                            cell->color);
            break;
        case QUEEN:
            return new Queen(cell->position.row, cell->position.column,
                             cell->color);
            break;
        case KNIGHT:
            return new Knight(cell->position.row, cell->position.column,
                              cell->color);
            break;
        case BISHOP:
            return new Bishop(cell->position.row, cell->position.column,
                              cell->color);
            break;
        case ROOK:
            return new Rook(cell->position.row, cell->position.column,
                            cell->color);
            break;
        case PAWN:
            return new Pawn(cell->position.row, cell->position.column,
                            cell->color);
            break;
        default:
            return NULL;
...
BoardInfo::BoardInfo(bool CWKS, bool CWQS, bool CBKS, bool CBQS, bool WC,
                     bool BC, Position eP) {
